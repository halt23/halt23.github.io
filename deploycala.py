...
def yaourt_update(noupgrade):
    ...
    if noupgrade:
        subprocess.call(["bash", "-c", "yaourt -Sy --noconfirm --needed --force" + " ".join(packages)], shell=False)
    else:
        subprocess.call(["bash", "-c", "yaourt -Syu --noconfirm --needed --force" + " ".join(packages)], shell=False)
