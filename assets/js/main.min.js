...
// Calamares Releases
  function GetLatestReleaseInfo() {
        $.getJSON("https://api.github.com/repos/calamares/calamares/releases/latest").done(function(release) {
            ...
            var date = new Date(release.created_at);
            var releaseInfo = release.name + " was released " + date.toDateString() + " and downloaded " + downloadCount.toLocaleString() + " times.";
...
// End Calamares Releases
// Extension Releases
  function GetLatestExtInfo() {
        $.getJSON("https://api.github.com/repos/calamares/calamares-extensions/releases/latest").done(function(extRelease) {
            var extAsset = extRelease.assets[0];
            var extCount = 0;
            for (var i = 0; i < extRelease.assets.length; i++) {
            extCount += extRelease.assets[i].download_count;
            }
            var extDate = new Date(extRelease.created_at);
            var extInfo = extRelease.name + " was released " + extDate.toDateString() + " and downloaded " + extCount.toLocaleString() + " times.";
            var extTagInfo = "Extensions " + extRelease.tag_name;
            $(".ext-download").attr("href", extAsset.browser_download_url);
            $(".ext-info").text(extInfo);
            $(".ext-info").fadeIn("slow");
            $(".ext-tag-info").text(extTagInfo);
        });
    }
